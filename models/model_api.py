    ...
    try:
        if name == "bash":
            ...
            if ext in text_exts:
                ...
                try:
                    import subprocess
                    result = subprocess.run(['python3', '-c', f'import PyPDF2; reader=PyPDF2.PdfReader("{fpath}"); print("\n".join([p.extract_text()[:5000] for p in reader.pages[:10]]))'], capture_output=True, text=True, timeout=15)
                    if result.returncode == 0:
...
def classify_intent(prompt):
    ...
    # Tool discovery
    if re.search(r'\b(find.+tool|search.+tool|what.+tools|available.+tool|best.+tool|ai tool|dev tool|cli tool|ide|which.+tool)\b', t):
        return ("tool_search", "ollama", "qwen2.5:7b", "Tool discovery query")
    # API discovery
    if re.search(r'\b(find.+api|search.+api|what.+api|available.+api|free.+api|which.+api)\b', t):
        return ("api_search", "ollama", "qwen2.5:7b", "API discovery query")
    # Free LLM providers
    if re.search(r'\b(free.+llm|free.+model|free.+gpt|free.+claude|free.+api|no.+cost.+model)\b', t):
        return ("freellm", "ollama", "qwen2.5:7b", "Free LLM provider search")
    # Learning hub
    if re.search(r'\b(learn|tutorial|course|guide|how to build|teach me)\b', t):
        return ("learning", "ollama", "qwen2.5:7b", "Learning hub query")
    ...
    if re.search(r'\b(code|app|api|function|script|build|react|python|javascript|html|css|deploy|debug|refactor|sql|database|backend|frontend|server)\b', t):
        return ("code", "groq", "openai/gpt-oss-120b", "Coding task — using Groq for speed")
    ...
    if re.search(r'\b(analyze|review|audit|check|inspect|read|file|upload|attachment)\b', t):
        return ("analysis", "groq", "openai/gpt-oss-120b", "File analysis — using Groq for speed")
    ...
    if re.search(r'\b(why|how|explain|compare|analyze|strategy|plan|architect|design pattern|best practice)\b', t):
        ...
        return ("reasoning", "groq", "openai/gpt-oss-120b", "Complex reasoning — using Groq")
    
    # Simple chat
    return ("chat", "groq", "qwen/qwen3.6-27b", "General chat — using Groq for speed")
...
def call_groq_with_tools(model, messages, tools, stream=False):
    """Call Groq API with tool calling support (OpenAI-compatible format)"""
    ...
    req = urllib.request.Request(
        ...
        data=data,
        headers={"Content-Type": "application/json", "Authorization": "Bearer " + GROQ_KEY, "User-Agent": "EvolvixOS/9.2"}
    )

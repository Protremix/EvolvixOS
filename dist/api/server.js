...
class BlockchainAPI {
    constructor(blockchain, walletManager, contractManager) {
        ...
        // Serve static images and icons
        this.app.use('/img', express_1.default.static(path_1.default.resolve(__dirname, '../web/img')));
        this.app.use('/download', express_1.default.static('/var/www/verdischain.com/download'));
  ...
  <div class="footer">
    ...
    <p style="margin-top:0.5rem">Built with @noble/secp256k1 and @noble/hashes. No mocks. No third-party dependencies.</p>
        ...
        this.app.get("/download/verdis-wallet.apk", (req, res) => {
            const apkPath = path_1.default.resolve(__dirname, "../web/verdis-wallet-v1.0.0.apk");
            if (fs_1.default.existsSync(apkPath)) {
                res.setHeader("Content-Type", "application/vnd.android.package-archive");
                res.setHeader("Content-Disposition", "attachment; filename=VerdisWallet-v1.0.0.apk");
        ...
        this.app.get("/api/wallet/app-info", (req, res) => {
            res.json({
                name: "Verdis Wallet",
                version: "1.0.0",
                packageId: "com.verdis.wallet",
                minAndroid: 7.0,
                targetAndroid: 14,
                size: "6.2 MB",
                downloadUrl: "https://verdischain.com/download/verdis-wallet.apk",
                features: [
                    "Native VCO wallet with secp256k1 cryptography",
                    "Send/Receive with QR codes",
                    "AMM DEX trading (VerdisSwap)",
                    "Eco impact tracking",
                    "DPoS validator staking",
                    "Secure local key storage",
                    "Carbon credit viewing"
                ]
            });
        });
...
// === Download page ===
        ...
        this.app.get('/token-sale', (req, res) => {
            const p = path_1.default.resolve(__dirname, '../web/token-sale.html');
            if (fs_1.default.existsSync(p)) { res.sendFile(p); return; }
            res.status(404).send('Token sale not found');
        });
        this.app.get('/bridge', (req, res) => {
            const p = path_1.default.resolve(__dirname, '../web/bridge.html');
            if (fs_1.default.existsSync(p)) { res.sendFile(p); return; }
            res.status(404).send('Bridge page not found');
        });
        this.app.get('/markets', (req, res) => {
            const p = path_1.default.resolve(__dirname, '../web/markets.html');
            if (fs_1.default.existsSync(p)) { res.sendFile(p); return; }
            res.status(404).send('Markets page not found');
        });
        this.app.get('/explorer', (req, res) => {
            const p = path_1.default.resolve(__dirname, '../web/explorer.html');
            if (fs_1.default.existsSync(p)) { res.sendFile(p); return; }
            res.status(404).send('Explorer page not found');
        });
        // === TPS endpoint ===
        ...
        // Pre-signed transaction submission (for native wallet apps)
        this.app.post('/api/transaction/submit', this.strictRateLimit.bind(this), (req, res) => {
            const tx = req.body;
            // Validate required fields
            if (!tx.from || !tx.to || !tx.signature || !tx.publicKey) {
                res.status(400).json({ error: 'Missing required fields: from, to, signature, publicKey' });
                return;
            }
            if (!tx.id) {
                // Recompute tx id if not provided
                const payload = `${tx.from}:${tx.to}:${tx.amount}:${tx.fee}:${tx.nonce}:${tx.data || ''}`;
                tx.id = (0, crypto_1.sha256)(payload);
            }
            ...
            res.json({ txId: tx.id, transaction: tx, success: true });
        });

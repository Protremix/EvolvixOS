...
class SendFragment : Fragment() {
        ...
        btnMax.setOnClickListener {
            ...
            lifecycleScope.launch {
                try {
                    ...
                    val balVrdx = balanceResp.balance 
                    val max = balVrdx - 0.001 // minus fee
            ...
            lifecycleScope.launch {
                try {
                    ...
                    if (result?.success == true) {
                        Toast.makeText(context, "Sent ${amount} VRDX ✓", Toast.LENGTH_LONG).show()
                        etAddress.setText("")
                        etAmount.setText("")
                    } else {
    ...
    private fun loadBalance() {
        ...
        lifecycleScope.launch {
            try {
                val balanceResp = withContext(Dispatchers.IO) { VerdisApi.getBalance(wallet.address) }
                val balVrdx = (balanceResp.balance as Number).toDouble() 
                tvAvailable.text = "Avail: ${String.format("%.4f", balVrdx)}"
            } catch (e: Exception) {}
        }
    }
}

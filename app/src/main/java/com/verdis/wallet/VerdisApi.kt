    ...
    companion object {
        ...
        suspend fun getDexPools(): List<DexPool> = withContext(Dispatchers.IO) {
            try {
                ...
                listOf(
                    DexPool("CARBON/VRDX", "CARBON", "VRDX", 1282593.66, 1561859.99, 1.21, 48559.0, 80, 3123719.99),
                    DexPool("ECO/VRDX", "ECO", "VRDX", 1499528.79, 1337675.19, 0.89, 73235.0, 93, 2999057.58),
...
suspend fun getEvmContracts(): List<EvmContract>? {
    return try {
        val response = rpcRequest("evm_contracts", emptyList())
        response?.getAsJsonArray("result")?.map {
            val obj = it.asJsonObject
            EvmContract(
                name = obj.get("name")?.asString ?: "Unknown",
                address = obj.get("address")?.asString ?: "0x0"
            )
        }
    } catch (e: Exception) { null }
}

suspend fun deployEvmContract(templateFile: String): Boolean {
    return try {
        rpcRequest("evm_deploy", listOf(templateFile))
        true
...
// --- AegisOS Integration Methods ---

suspend fun getAegisOSHealth(): AegisOSHealth {
    return try {
        ...
        if (response.isSuccessful) {
            ...
            AegisOSHealth(
                ...
                agents = json.get("agents_active")?.asInt ?: 11
            )
        } else {
            AegisOSHealth("offline", 0)
        }
    } catch (e: Exception) {
        AegisOSHealth("offline", 0)
    }
